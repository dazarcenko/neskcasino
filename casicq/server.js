const express = require('express');
const path = require('path');
const { Telegraf, Markup } = require('telegraf');

const app = express();

app.use(express.json());
app.use(express.static(path.join(__dirname, 'public')));

const bot = new Telegraf(process.env.BOT_TOKEN);

const WEB_APP_URL = process.env.WEB_APP_URL;
const CHANNEL_ID = process.env.CHANNEL_ID;

const ADMIN_ID = process.env.ADMIN_ID || '1067205524';


// =====================================================
// БАЗА ДАННЫХ
// =====================================================

const db = {};


// =====================================================
// ПРИЗЫ
// =====================================================

const PRIZE_NAMES = {
    'images/zhiza.jpg': 'ЖИЖА',
    'images/vape.jpg': 'ПОД-СИСТЕМА',
    'images/odnorazka.jpg': 'ОДНОРАЗКА',
    '⭐': 'СЕКРЕТНЫЙ БОКС',
    'N': 'ПРОМОКОД'
};


// =====================================================
// СИМВОЛЫ БАРАБАНА
// =====================================================

const SYMBOLS = [
    'images/zhiza.jpg',
    'images/vape.jpg',
    'images/odnorazka.jpg',
    '⭐',
    'N'
];


// =====================================================
// СЛУЧАЙНЫЙ СИМВОЛ
// =====================================================

function getRandom() {
    return SYMBOLS[
        Math.floor(Math.random() * SYMBOLS.length)
    ];
}


// =====================================================
// ДРУГОЙ СИМВОЛ
// =====================================================

function getDiff(excluded) {
    let symbol;

    do {
        symbol = getRandom();
    } while (symbol === excluded);

    return symbol;
}


// =====================================================
// КОМАНДА /START
// =====================================================

bot.start(async (ctx) => {

    const userId = String(ctx.from.id);


    // ---------------------------------------------
    // СОЗДАЁМ ПОЛЬЗОВАТЕЛЯ
    // ---------------------------------------------

    if (!db[userId]) {
        db[userId] = {
            spins: 0,
            hasClaimedWelcome: false,
            referredBy: null,
            referrals: 0
        };
    }


    // ---------------------------------------------
    // ПОЛУЧАЕМ РЕФЕРАЛЬНЫЙ ID
    // ---------------------------------------------

    const startPayload = ctx.startPayload;

    const referrerId = startPayload
        ? String(startPayload)
        : null;


    // ---------------------------------------------
    // ОБРАБОТКА РЕФЕРАЛА
    // ---------------------------------------------

    if (
        referrerId &&
        referrerId !== userId
    ) {

        // Создаём реферера, если его ещё нет
        if (!db[referrerId]) {
            db[referrerId] = {
                spins: 0,
                hasClaimedWelcome: false,
                referredBy: null,
                referrals: 0
            };
        }


        // Проверяем, не был ли пользователь
        // уже приглашён кем-то
        if (!db[userId].referredBy) {

            // Запоминаем, кто его пригласил
            db[userId].referredBy = referrerId;

            // Пригласившему +3 спина
            db[referrerId].spins += 3;

            // Увеличиваем количество рефералов
            db[referrerId].referrals += 1;


            console.log(
                `РЕФЕРАЛ: ${referrerId} пригласил ${userId}. +3 спина`
            );


            // -----------------------------------------
            // УВЕДОМЛЕНИЕ ПРИГЛАСИВШЕМУ
            // -----------------------------------------

            try {

                await bot.telegram.sendMessage(
                    referrerId,

                    `🎉 По твоей ссылке зашёл новый пользователь!\n\n` +
                    `👤 ${ctx.from.first_name || 'Новый игрок'}\n` +
                    `🎰 Тебе начислено +3 спина!`
                );

            } catch (err) {

                console.error(
                    'Ошибка отправки уведомления рефереру:',
                    err.description || err.message
                );

            }
        }
    }


    // ---------------------------------------------
    // ПРИВЕТСТВЕННОЕ СООБЩЕНИЕ
    // ---------------------------------------------

    await ctx.reply(

        '🎰 Добро пожаловать в NeskShop Slots!\n\n' +
        'Жми кнопку ниже, чтобы начать игру:',

        Markup.inlineKeyboard([
            Markup.button.webApp(
                '🎰 ИГРАТЬ',
                WEB_APP_URL
            )
        ])

    );

});


// =====================================================
// ДАННЫЕ ПОЛЬЗОВАТЕЛЯ
// =====================================================

app.post('/api/user-data', async (req, res) => {

    const { userId } = req.body;


    // ---------------------------------------------
    // СОЗДАЁМ ПОЛЬЗОВАТЕЛЯ
    // ---------------------------------------------

    if (!db[userId]) {

        db[userId] = {
            spins: 0,
            hasClaimedWelcome: false,
            referredBy: null,
            referrals: 0
        };

    }


    let isSubbed = false;


    // ---------------------------------------------
    // ПРОВЕРКА ПОДПИСКИ
    // ---------------------------------------------

    try {

        if (CHANNEL_ID) {

            const member =
                await bot.telegram.getChatMember(
                    CHANNEL_ID,
                    userId
                );


            isSubbed = [
                'member',
                'administrator',
                'creator'
            ].includes(member.status);

        }

    } catch (e) {

        console.error(
            `Ошибка проверки подписки для ${userId}:`,
            e.description || e.message
        );

    }


    // ---------------------------------------------
    // 5 СТАРТОВЫХ СПИНОВ
    // ---------------------------------------------

    if (
        isSubbed &&
        !db[userId].hasClaimedWelcome
    ) {

        db[userId].spins += 5;

        db[userId].hasClaimedWelcome = true;

    }


    // ---------------------------------------------
    // ОТВЕТ MINI APP
    // ---------------------------------------------

    res.json({
        isSubbed: isSubbed,
        spins: db[userId].spins
    });

});


// =====================================================
// SPIN
// =====================================================

app.post('/api/spin', async (req, res) => {

    const {
        userId,
        firstName
    } = req.body;


    // ---------------------------------------------
    // ПРОВЕРКА СПИНОВ
    // ---------------------------------------------

    if (
        !db[userId] ||
        db[userId].spins < 1
    ) {

        return res.status(400).json({
            error: 'Нет спинов!'
        });

    }


    // ---------------------------------------------
    // СПИСЫВАЕМ 1 СПИН
    // ---------------------------------------------

    db[userId].spins -= 1;


    // ---------------------------------------------
    // РАНДОМ
    // ---------------------------------------------

    const roll = Math.random();


    let result = [];

    let winAmount = 0;

    let prizeName = null;


    // =================================================
    // ДЖЕКПОТ — 0.1%
    // =================================================

    if (roll < 0.001) {

        const symbol = getRandom();


        result = [
            symbol,
            symbol,
            symbol
        ];


        // +10 спинов за джекпот
        winAmount = 10;


        prizeName =
            PRIZE_NAMES[symbol] || 'ПРИЗ';


        // ---------------------------------------------
        // УВЕДОМЛЕНИЕ АДМИНУ
        // ---------------------------------------------

        try {

            await bot.telegram.sendMessage(

                ADMIN_ID,

                `🚨 <b>НОВЫЙ ВЫИГРЫШ!</b>\n\n` +

                `👤 Игрок: ${firstName || 'Неизвестный'} ` +
                `(ID: <code>${userId}</code>)\n\n` +

                `🎁 Приз: <b>${prizeName}</b>\n` +

                `🎰 Комбинация: 3x ${prizeName}`,

                {
                    parse_mode: 'HTML'
                }

            );

        } catch (err) {

            console.error(
                'Ошибка отправки сообщения админу:',
                err.description || err.message
            );

        }

    }


    // =================================================
    // ПАРА — 30%
    // =================================================

    else if (roll < 0.301) {

        const symbol1 = getRandom();

        const symbol2 = symbol1;

        const symbol3 = getDiff(symbol1);


        result = [
            symbol1,
            symbol2,
            symbol3
        ];


        winAmount = 0;

    }


    // =================================================
    // ПРОИГРЫШ — 69.9%
    // =================================================

    else {

        const symbol1 = getRandom();

        const symbol2 = getDiff(symbol1);


        let symbol3;


        do {

            symbol3 = getRandom();

        } while (
            symbol3 === symbol1 ||
            symbol3 === symbol2
        );


        result = [
            symbol1,
            symbol2,
            symbol3
        ];


        winAmount = 0;

    }


    // ---------------------------------------------
    // ДОБАВЛЯЕМ ВЫИГРАННЫЕ СПИНЫ
    // ---------------------------------------------

    db[userId].spins += winAmount;


    // ---------------------------------------------
    // ОТДАЁМ РЕЗУЛЬТАТ MINI APP
    // ---------------------------------------------

    res.json({

        result: result,

        winAmount: winAmount,

        prizeName: prizeName,

        spins: db[userId].spins

    });

});


// =====================================================
// ЗАПУСК БОТА
// =====================================================

bot.launch()
    .then(() => {

        console.log(
            'Бот успешно запущен!'
        );

    })
    .catch((err) => {

        console.error(
            'Ошибка запуска бота:',
            err
        );

    });


// =====================================================
// ЗАПУСК СЕРВЕРА
// =====================================================

const PORT =
    process.env.PORT || 3000;


app.listen(
    PORT,
    () => {

        console.log(
            `Сервер работает на порту ${PORT}`
        );

    }
);


// =====================================================
// ЗАВЕРШЕНИЕ
// =====================================================

process.once(
    'SIGINT',
    () => bot.stop('SIGINT')
);

process.once(
    'SIGTERM',
    () => bot.stop('SIGTERM')
);
